...
class MetaverseRoom extends Room {
  ...
  onJoin(client, options) {
    ...
    try {
      ...
      player.avatarUrl =
        options.avatarUrl && options.avatarUrl !== 'default' ? options.avatarUrl : '';
      player.lastUpdate = Date.now();
      ...
      console.log(
        `✅ Player ${client.sessionId} (${player.userId}) joined. Total players: ${this.state.players.size}`
      );
    } catch (error) {
